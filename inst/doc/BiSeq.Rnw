...
\label{import}
Bismark \cite{Krueger2011} is a tool to map bisulfite converted sequencing reads and to determine cytosine methylation states. \Rpackage{BiSeq} allows the import of Bismark output files. 
<<preliminaries>>=
library(BiSeq)
@
...
@
The arguments \Rfunarg{otFiles} and \Rfunarg{obFiles} point to Bismark's output files for the positive (original top) and the negative (original bottom) strand respectively. \Rfunarg{colData} specifies the sample names and additional phenotype information. This method returns a \Rclass{BSraw} object.
